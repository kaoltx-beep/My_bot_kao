...
def integrate() -> None:
    ...
    if CALLBACK_MARK not in source:
        ...
        start = source.find("\n@bot.callback_query_handler(func=lambda call: call.data.startswith(\"tool_approval:\"))")
        end = source.find("\n@bot.message_handler(func=lambda m: True)", start)
        if start >= 0 and end > start:
            source = source[:start] + CALLBACK_BLOCK + source[end:]
